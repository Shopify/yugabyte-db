...
DEFINE_RUNTIME_bool(otel_trace_catalog_version_check, false,
    "Trace the periodic lagging-catalog-version check under a root span; the internal PG "
    "connection's backend RPCs nest beneath it. When false the root is suppressed.");

DEFINE_RUNTIME_bool(otel_trace_relcache_init, false,
    "Trace each relcache-init connection under a root span; the InitPostgres catalog RPCs nest "
    "beneath it. When false the root is suppressed.");
...
Status TabletServer::CheckYsqlLaggingCatalogVersions() {
  ...
  // Open a root span for catalog version check.
  auto trace_span = dist_trace::StartOriginRootSpanWithScope(
