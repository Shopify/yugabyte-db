...
METRIC_DEFINE_gauge_uint32(server, ts_index_backfill_pinned_tablets,
    "Tablets Pinned by an Index-Backfill Retention Barrier", MetricUnit::kUnits,
    "Number of tablet peers on this TServer whose history GC is pinned by an active "
...
METRIC_DEFINE_gauge_uint64(server, ts_index_backfill_oldest_retention_barrier_age_ms,
    ...
    "bound on how much extra history the most-pinned tablet is retaining. 0 when no "
...
// Retention-pin observability (#33444): with the verification retention barrier enforced in
...
// generation regardless of peer state -- the pin is on-disk metadata, not runtime state.
void TSTabletManager::EmitIndexBackfillRetentionMetrics() {
  uint32_t pinned_tablets = 0;
  ...
  index_backfill_pinned_tablets_metric_->set_value(pinned_tablets);
...
TSTabletManager::TSTabletManager(FsManager* fs_manager,
  ...
  index_backfill_pinned_tablets_metric_ =
      METRIC_ts_index_backfill_pinned_tablets.Instantiate(server_->metric_entity(), 0);
