 ...
 protected:
  ...
  Result<OpId> SendMarkedWrite(
    ...
    for (const auto& [key, value] : kvs) {
      // Hash 0 rather than the real hash: the single tablet covers the entire hash range, and the
      // test reads back through DocDB dumps, which do not re-derive hashes.
      const DocKey doc_key(0, MakeKeyEntryValues(key));
