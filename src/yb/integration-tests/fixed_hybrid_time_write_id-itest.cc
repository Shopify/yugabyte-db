...
namespace tserver {
METRIC_DECLARE_gauge_uint32(ts_index_backfill_pinned_tablets);
...
// Retention-pin observability: the ts_index_backfill_* gauges surface active generations
// (pinned-tablet count) and the oldest barrier's age, so a stuck barrier is findable without
// reading superblocks. Each tserver hosts one replica of the single tablet here, so the
// per-server pinned count is 0 or 1.
TEST_F(FixedHybridTimeWriteIdITest, RetentionPinMetricsTrackActiveGenerations) {
  auto leaders = ASSERT_RESULT(WaitForTableActiveTabletLeadersPeers(
      cluster_.get(), table_.table()->id(), /* num_active_leaders= */ 1));
  const auto tablet_id = leaders.front()->tablet_id();

  const auto pinned_tablets = [this](size_t ts_idx) -> Result<uint32_t> {
    const auto metric =
        cluster_->mini_tablet_server(ts_idx)->metric_entity().FindOrNull<AtomicGauge<uint32_t>>(
            tserver::METRIC_ts_index_backfill_pinned_tablets);
  ...
  for (size_t i = 0; i != cluster_->num_tablet_servers(); ++i) {
    ASSERT_EQ(0, ASSERT_RESULT(pinned_tablets(i)));
  ...
  for (size_t i = 0; i != cluster_->num_tablet_servers(); ++i) {
    ASSERT_OK(LoggedWaitFor(
        [&]() -> Result<bool> { return VERIFY_RESULT(pinned_tablets(i)) == 1; },
        MonoDelta::FromSeconds(15) * kTimeMultiplier,
        Format("pinned-tablet gauge to reach 1 on ts-$0", i)));
    ASSERT_GE(ASSERT_RESULT(oldest_age_ms(i)), kBarrierAgeSec * 1000ULL - 500);
  }

  // Active without a barrier still counts as pinned (it blocks all history GC) but
  ...
  for (size_t i = 0; i != cluster_->num_tablet_servers(); ++i) {
    ...
    ASSERT_EQ(1, ASSERT_RESULT(pinned_tablets(i)));
  }
  ...
  for (size_t i = 0; i != cluster_->num_tablet_servers(); ++i) {
    ASSERT_OK(LoggedWaitFor(
        [&]() -> Result<bool> { return VERIFY_RESULT(pinned_tablets(i)) == 0; },
        MonoDelta::FromSeconds(15) * kTimeMultiplier,
        Format("pinned-tablet gauge to return to 0 on ts-$0", i)));
    ASSERT_EQ(0, ASSERT_RESULT(oldest_age_ms(i)));
  }
}
