...
// Regression test for a bug where the packed row intent following a tombstone for the same key
// inherited the DELETE op instead of starting a new INSERT record, because both intents shared
// the same physical hybrid time (same write batch).
