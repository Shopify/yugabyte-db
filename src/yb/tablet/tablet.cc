...
void Tablet::DoCleanupIntentFiles() {
  ...
  while (FLAGS_cleanup_intents_sst_files) {
    ...
    // Checkpoint-less (StreamWAL) CDC wall-clock intent retention. There is no registered CDC SDK
    // stream (is_under_cdc_sdk_replication() is false), so the stream barrier above does not
