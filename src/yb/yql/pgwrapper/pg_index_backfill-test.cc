...
TEST_P(PgIndexBackfillSkipAllBlocked, SplitFencedDuringBackfill) {
  ...
  // -- only the backfill-job check can reject, and it logs the dedicated skip reason.
  auto fence_log_waiter =
      cluster_->GetMasterLogWaiter("Skipping tablet splitting for index table");
  const auto split_status = cluster_->CallYbAdmin({"split_tablet", index_tablet_id});
  ASSERT_NOK(split_status);
  ASSERT_OK(fence_log_waiter.WaitFor(MonoDelta::FromSeconds(10)));
...
// Retention-pin gauges under the production lifecycle: pinned while the generation is held.
...
TEST_P(PgIndexBackfillVerifier, RetentionPinMetricsReflectHeldGeneration) {
  ASSERT_OK(conn_->ExecuteFormat("CREATE TABLE $0 (a int PRIMARY KEY, b int)", kTableName));
  ASSERT_OK(conn_->ExecuteFormat("INSERT INTO $0 VALUES (1, 1)", kTableName));
  ASSERT_OK(conn_->ExecuteFormat(
      "CREATE UNIQUE INDEX $0 ON $1 (b HASH) SPLIT INTO 1 TABLETS", kIndexName, kTableName));
  ...
  // tserver must report the pin, with a positive barrier age (the barrier derives from the
  // backfill read time, which is necessarily in the past by build completion).
  for (size_t i = 0; i != cluster_->num_tablet_servers(); ++i) {
    auto* ts = cluster_->tablet_server(i);
    ASSERT_OK(WaitFor(
        [ts]() -> Result<bool> {
          return VERIFY_RESULT(ts->GetMetric<int64>(
              "server", "yb.tabletserver", "ts_index_backfill_pinned_tablets", "value")) >= 1;
        },
        MonoDelta::FromSeconds(15) * kTimeMultiplier,
        Format("tserver $0 to report a pinned tablet", i)));
    ASSERT_GT(
...
TEST_P(PgIndexBackfillVerifierReleased, RetentionPinMetricsClearOnRelease) {
  std::vector<ExternalDaemon*> tserver_daemons;
  for (auto* tserver : cluster_->tserver_daemons()) {
    tserver_daemons.push_back(tserver);
  }
  ...
  for (size_t i = 0; i != cluster_->num_tablet_servers(); ++i) {
    ...
    ASSERT_OK(WaitFor(
        [ts]() -> Result<bool> {
          return VERIFY_RESULT(ts->GetMetric<int64>(
              "server", "yb.tabletserver", "ts_index_backfill_pinned_tablets", "value")) == 0;
        },
        MonoDelta::FromSeconds(15) * kTimeMultiplier,
        Format("tserver $0 pinned-tablet gauge to clear", i)));
  }
}
...
TEST_P(PgIndexBackfillSnapshotTooOld, SnapshotTooOldLegacy) {
  auto client = ASSERT_RESULT(cluster_->CreateClient());
  constexpr int kTimeoutSec = 3;
...
INSTANTIATE_TEST_CASE_P(, PgIndexBackfillSnapshotTooOldPackedRowsOff, ::testing::Bool());
...
TEST_P(PgIndexBackfillSnapshotTooOldPackedRowsOff, BelowHistoryCutoffReadFailsLoud) {
  auto client = ASSERT_RESULT(cluster_->CreateClient());
  constexpr int kTimeoutSec = 3;
...
TEST_P(
    PgIndexBackfillCancellationTest,
  ...
  if (EnableTableLocks()) {
    // Wait for session expiry so that the locks are released.
    SleepFor(kPgClientSessionExpiration * kTimeMultiplier * 2);
  }
